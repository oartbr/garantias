...
import { GarantiaFilterType } from "./garantia-filter-types";

type GarantiaFilterFormData = GarantiaFilterType;

function GarantiaFilter() {
  const { t } = useTranslation("admin-panel-garantias");
  const router = useRouter();
  const searchParams = useSearchParams();

  const methods = useForm<GarantiaFilterFormData>({
    defaultValues: {
      status: [],
    },
  });
